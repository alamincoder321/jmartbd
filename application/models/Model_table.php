...
class Model_Table extends CI_Model
{
    ...
    public function generateInternationalPurchaseInvoice()
    {
        $invoice = date('Y') . "000001";
        $year = date('Y');
        $purchases = $this->db->query("select * from tbl_international_purchasemaster pm where pm.PurchaseMaster_InvoiceNo like '$year%'");
        if ($purchases->num_rows() != 0) {
            $newPurchaseId = $purchases->num_rows() + 1;
            $zeros = array('0', '00', '000', '0000', '00000');
            $invoice = date('Y') . (strlen($newPurchaseId) > count($zeros) ? $newPurchaseId : $zeros[count($zeros) - strlen($newPurchaseId)] . $newPurchaseId);
    ...
    public function generateInternationalSupplierCode()
    {
        $supplierCode = "S00001";

        $lastSupplier = $this->db->query("select * from tbl_international_supplier order by Supplier_SlNo desc limit 1");
        if ($lastSupplier->num_rows() != 0) {
            $newSupplierId = $lastSupplier->row()->Supplier_SlNo + 1;
            $zeros = array('0', '00', '000', '0000');
            $supplierCode = 'S' . (strlen($newSupplierId) > count($zeros) ? $newSupplierId : $zeros[count($zeros) - strlen($newSupplierId)] . $newSupplierId);
    ...
    public function generateInternationalSupplierPaymentCode()
    {
        $paymentCode = "TR00001";

        $lastPayment = $this->db->query("select * from tbl_international_supplier_payment order by SPayment_id desc limit 1");
        if ($lastPayment->num_rows() != 0) {
            $newPaymentId = $lastPayment->row()->SPayment_id + 1;
            $zeros = array('0', '00', '000', '0000');
            $paymentCode = 'TR' . (strlen($newPaymentId) > count($zeros) ? $newPaymentId : $zeros[count($zeros) - strlen($newPaymentId)] . $newPaymentId);
    ...
    public function generateCashTransferInvoice()
    {
        $transactionCode = "TR00001";

        $lastTransaction = $this->db->query("select * from tbl_cash_transfer order by id desc limit 1");
        if ($lastTransaction->num_rows() != 0) {
            $newTransactionId = $lastTransaction->row()->id + 1;
            $zeros = array('0', '00', '000', '0000');
            $transactionCode = 'TR' . (strlen($newTransactionId) > count($zeros) ? $newTransactionId : $zeros[count($zeros) - strlen($newTransactionId)] . $newTransactionId);
        }
    ...
    public function getTransactionSummary($date = null, $branchId = null)
    {
        ...
        $transactionSummary = $this->db->query("
            ...
            (
                ...
                where ex.Status = 'a'
                " . (!empty($branchId) ? "and ex.branchId = '$branchId'" : "") . "
            ...
            (
                ...
                " . (!empty($branchId) ? "and sp.SPayment_brunchid = '$branchId'" : "") . "
            ...
            (
                ...
                where la.status = 1
                " . (!empty($branchId) ? "and la.branch_id = '$branchId'" : "") . "
            ...
            (
                ...
                where ass.status = 'a'
                " . (!empty($branchId) ? "and ass.branchid = '$branchId'" : "") . "
        ...
        $bankTransactionSummary = $this->db->query("
            select 
                ...
                (
                    ...
                    and smb.Status = 'a'
                    " . (!empty($branchId) ? "and smb.branchId = '$branchId'" : "") . "
                    " . ($date == null ? "" : " and sm.SaleMaster_SaleDate < '$date'") . "
                ) as sale_receive,
            ...
            where 1 = 1
            " . (!empty($branchId) ? "and ba.branch_id = '$branchId'" : "") . " 
    ...
    public function assetsReport($clauses = '', $date = null, $branchId = null)
    {
        ...
        $assets = $this->db->query("
            ...
            ( SELECT ifnull( sum(as_qty) , 0) 
                ...
                and buy_or_sale = 'buy'
                and status = 'a'
                " . (!empty($branchId) ? "and branchid = '$branchId'" : "") . "
    ...
    public function currentStock($clauses = '', $branchId = null)
    {
        ...
        $stock = $this->db->query("
            select * from(
                ...
                " . (!empty($branchId) ? "and ci.branch_id = '$branchId'" : "") . "
            ) as tbl
            where 1 = 1
            $clauses
        ")->result();
    ...
    public function customerDue($clauses = "", $date = null, $branchId = null)
    {
        ...
        $dueResult = $this->db->query("
            ...
            from tbl_customer c
            where c.status = 'a'
            " . (!empty($branchId) ? "and c.Customer_brunchid = '$branchId'" : "") . "
            $clauses
        ")->result();
