...
<script>
  new Vue({
    ...
    methods: {
      ...
      excelExport() {
        let onlyData = this.suppliers.map(item => {
          return {
            'Supplier Id': item.Supplier_Code,
            'Supplier Name': item.Supplier_Name,
            'Address': item.Supplier_Address ?? '' + ' ' + item.District_Name,
            'Contact No.': item.Supplier_Mobile
          }
        })
        ...
        // Excel download
        XLSX.writeFile(workbook, "SupplierList.xlsx");
      }
    }
  })
</script>