...
<div id="purchaseRecord">
	...
	<div class="row" style="margin-top:15px;display:none;" v-bind:style="{display: purchases.length > 0 ? '' : 'none'}">
		...
		<div class="col-md-5 text-right">
			<strong>Total Purchase: </strong> <span v-text="purchases.length"></span>
		</div>
				...
				<table
					...
					<tfoot>
						<tr style="font-weight:bold;">
							...
							<td style="text-align:right;">{{ purchases.reduce((prev, curr)=>{return prev + parseFloat(curr.PurchaseMaster_SubTotalAmount)}, 0).toFixed(5) }}</td>
							<td style="text-align:right;">{{ purchases.reduce((prev, curr)=>{return prev + parseFloat(curr.PurchaseMaster_Tax)}, 0).toFixed(5) }}</td>
							<td style="text-align:right;">{{ purchases.reduce((prev, curr)=>{return prev + parseFloat(curr.PurchaseMaster_DiscountAmount)}, 0).toFixed(5) }}</td>
							<td style="text-align:right;">{{ purchases.reduce((prev, curr)=>{return prev + parseFloat(curr.PurchaseMaster_Freight)}, 0).toFixed(5) }}</td>
							<td style="text-align:right;">{{ purchases.reduce((prev, curr)=>{return prev + parseFloat(curr.PurchaseMaster_TotalAmount)}, 0).toFixed(5) }}</td>
							<td style="text-align:right;">{{ purchases.reduce((prev, curr)=>{return prev + parseFloat(curr.PurchaseMaster_PaidAmount)}, 0).toFixed(5) }}</td>
							<td style="text-align:right;">{{ purchases.reduce((prev, curr)=>{return prev + parseFloat(curr.PurchaseMaster_DueAmount)}, 0).toFixed(5) }}</td>
							<td></td>
							<td></td>
						</tr>
					</tfoot>
			...
			excelExport() {
				let onlyData = this.purchases.map(item => {
					return {
						...
						'Sub Total': item.PurchaseMaster_SubTotalAmount ?? '',
						'VAT': item.PurchaseMaster_Tax ?? '',
						'Discount': item.PurchaseMaster_DiscountAmount ?? '',
						'Transport Cost': item.PurchaseMaster_Freight ?? '',
						'Total': item.PurchaseMaster_TotalAmount ?? item.purchaseDetails.reduce((prev, curr) => {
							return prev + parseFloat(curr.PurchaseDetails_TotalAmount)
						}, 0),
						'Paid': item.PurchaseMaster_PaidAmount ?? '',
						'Due': item.PurchaseMaster_DueAmount ?? '',
