			...
			<table class="table table-bordered">
				...
				<tbody>
					...
					<tr>
						...
						<td style="text-align:right;font-weight:700;">{{ payments.reduce((prev, curr) => {return prev + parseFloat(curr.bill)}, 0).toFixed(2) }}</td>
						<td style="text-align:right;font-weight:700;">{{ payments.reduce((prev, curr) => {return prev + parseFloat(curr.paid)}, 0).toFixed(2) }}</td>
						<td style="text-align:right;font-weight:700;">{{ payments.reduce((prev, curr) => {return prev + parseFloat(curr.due)}, 0).toFixed(2) }}</td>
						<td style="text-align:right;font-weight:700;">{{ parseFloat(balance).toFixed(2) }}</td>
					</tr>
				</tbody>
			</table>
		</div>
	...
	new Vue({
		...
		methods: {
			...
			getReport() {
				...
				axios.post('/get_international_supplier_ledger', data).then(res => {
					this.payments = res.data.payments;
					if (this.payments.length > 0) {
						this.balance = this.payments.slice(-1)[0].balance;
					}
			...
			async print() {
				let reportContent = `
					<div class="container">
						<h4 style="text-align:center">Supplier Ledger</h4 style="text-align:center">
						<div class="row">
