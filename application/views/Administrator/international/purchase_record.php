				...
				<table class="record-table" v-if="(searchTypesForRecord.includes(searchType)) && recordType == 'with_details'" style="display:none" v-bind:style="{display: (searchTypesForRecord.includes(searchType)) && recordType == 'with_details' ? '' : 'none'}">
					...
					<tbody>
						<template v-for="purchase in purchases">
							<tr>
								...
								<td style="text-align:center;">
									<a href="" title="Purchase Invoice" v-bind:href="`/international_purchase_invoice_print/${purchase.PurchaseMaster_SlNo}`" target="_blank"><i class="fa fa-file-text"></i></a>
						...
						<tr style="font-weight:bold;background: lightgrey;">
							<td colspan="5" style="font-weight:normal;"></td>
							<td style="text-align:center;">Total Quantity<br>{{ parseInt(purchases.reduce((prev, curr) => {return prev + curr.purchaseDetails.reduce((pre, cur) => {return pre + parseFloat(cur.PurchaseDetails_TotalQuantity)}, 0)}, 0)) }}</td>
							<td style="text-align:right;">
								Total: {{ purchases.reduce((prev, curr) => {return prev + parseFloat(curr.PurchaseMaster_TotalAmount)}, 0).toFixed(2) }}<br>
								Paid: {{ purchases.reduce((prev, curr) => {return prev + parseFloat(curr.PurchaseMaster_PaidAmount)}, 0).toFixed(2) }}<br>
								Due: {{ purchases.reduce((prev, curr) => {return prev + parseFloat(curr.PurchaseMaster_DueAmount)}, 0).toFixed(2) }}
							</td>
							<td></td>
						</tr>
					</tbody>
	...
	new Vue({
		...
		methods: {
			checkReturnAndEdit(purchase) {
				location.replace('/international_purchase/' + purchase.PurchaseMaster_SlNo);
			},
			onChangeSearchType() {
				this.purchases = [];
			...
			getPurchaseRecord() {
				...
				let url = '/get_international_purchases';
				if (this.recordType == 'with_details') {
					url = '/get_international_purchase_record';
				}
			...
			getPurchaseDetails() {
				...
				axios.post('/get_international_purchasedetails', filter)
					.then(res => {
						this.purchases = res.data;
					})
			},
			deletePurchase(purchaseId) {
				...
				axios.post('/delete_international_purchase', {
