				...
				<datatable :columns="columns" :data="payments" :filter-by="filter" style="margin-bottom: 5px;">
					<template scope="{ row }">
						<tr>
							...
							<td>
								<button type="button" class="button edit" @click="window.location = `/internationalsupplierPaymentInvoice/${row.SPayment_id}`">
	...
	new Vue({
		...
		methods: {
			...
			saveSupplierPayment() {
				...
				let url = '/add_international_supplier_payment';
				if (this.payment.SPayment_id != 0) {
					url = '/update_international_supplier_payment';
				}
			...
			deletePayment(paymentId) {
				...
				axios.post('/delete_international_supplier_payment', {
					paymentId: paymentId
				}).then(res => {
					let r = res.data;
					alert(r.message);
